import { Header } from "antd/es/layout/layout";
import { NavLink } from "react-router-dom";

const Navbar = () => {
  return (
    <Header className="bg-[#DCFFCB]   h-16">
      <div className="flex     justify-center items-center gap-3 sm:gap-5 md:gap-7 lg:gap-10">
        <NavLink to={"/"}>
          <div className="relative  mt-2">
            <h1 className="text-lg md:text-xl lg:text2xl font-bold after:content-[''] after:bg-black after:h-[3px] after:w-[0%] after:left-0 after:-bottom-[5px] after:rounded-xl after:duration-300 after:absolute hover:after:w-[100%] ">
              Home
            </h1>
          </div>
        </NavLink>
        <NavLink to={"/products"}>
          <div className="relative  mt-2">
            <h1 className="text-lg md:text-xl lg:text2xl  font-bold after:content-[''] after:bg-black after:h-[3px] after:w-[0%] after:left-0 after:-bottom-[5px] after:rounded-xl after:duration-300 after:absolute hover:after:w-[100%] ">
              Product
            </h1>
          </div>
        </NavLink>
        <NavLink to={"/management"}>
          <div className="relative  mt-2">
            <h1 className="text-lg md:text-xl lg:text2xl  font-bold after:content-[''] after:bg-black after:h-[3px] after:w-[0%] after:left-0 after:-bottom-[5px] after:rounded-xl after:duration-300 after:absolute hover:after:w-[100%] ">
              Management
            </h1>
          </div>
        </NavLink>

        <NavLink to={"/cart"}>
          <div className="w-[44px] text-[#4d6429] relative top-1 h-[44px]  hover:scale-110">
            <img src="data:image/png;base64," />
          </div>
        </NavLink>
      </div>
    </Header>
  );
};

export default Navbar;
